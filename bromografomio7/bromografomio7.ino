...
//---Funzioni--------------------------------------------------------------------
  ...
  void checkOffLamp(){
    if(indiceMenu!=3&&digitalRead(pinchiuso)==HIGH){
      for(int i=0; i<4; i++){
...
void setup() {
  ...
  pinMode(pulsanti[0], INPUT);
  pinMode(pulsanti[1], INPUT);
  pinMode(pulsanti[2], INPUT);
  ...
  switch (indiceMenu){
    ...
    case 3:
     ...
     case 4:
       ...
       lcd.setCursor(0,0);
       printTime(rimanente);
       if(readPulsanti(pulsanteDx1)||digitalRead(pinchiuso)==LOW){
         indiceMenu=5;
         delay(delaypulsante);
         lcd.clear();
       }
